...
:::message
Rust には `.iter()`, `.iter_mut()`, `.into_iter()` の 3 種類のイテレータの取り出し方法があります。それぞれの意味は、

- `.iter`: 各要素を `&T` 型で返すイテレータ
- `.iter_mut`: 各要素を `&mut T` 型で返すイテレータ
- `.into_iter`: 各要素を `T` 型で返すイテレータ

となっています。

まだ紹介していませんが、Rust の中心概念には所有権というものがあります。3 つも種類があるのは、この所有権を効率よく扱うために必要になってくるからです。

`into_iter` をすると、そのコレクションの所有権をムーブすることになるので、一度 `into_iter` メソッドを呼ぶと、以降はそのコレクションにアクセスできなくなります。

他の 2 つは Rust の借用を使用しているので、一度呼び出した後も再びそのコレクションに対してアクセスすることは可能です。

このハンズオンでは、所有権と借用に関する話は後半に初めて登場させ、前半部分は Java などの高級プログラミング言語出身者向けに参照などは一切登場させないという構成を取っています。少し先取りした話になったので、今は「なんのことやら」という気がするかもしれません。所有権や借用を学んだあとでこのヒントをもう一度見直すと、すべてがつながるはずです。
:::

また、数値を羅列したい場合は `0..99` のような書き方をできます。これは `range` と呼ばれるもので、`[start, end)` (たとえば `i` という数値の取りうる範囲に関して、`i >= start`, `i < end`) な値を生成してくれます。これを使用すると、たとえば下記のような Java コードは、
...
もし終わりも閉区間（`[start, end]`）としたい場合は、`..=` とするとできます。

```rust
for i in 1..=100 {
    println!("{}", i); // 1〜"100"までが出力される
}
```

ちなみに、イテレータについては 7 章にて紹介予定です。
