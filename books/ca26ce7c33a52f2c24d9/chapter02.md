...
ちなみに、変数の命名規則はスネークケース (たとえば、`let snake_case = ...;`) です。
...
- `let` で変数宣言をできる。デフォルトはイミュータブルになる。
- `let mut` とすると、ミュータブルな変数宣言をできる。
- Rust では型推論が強く効くので、型注釈は省略してコードを書くことができる。
...
最後の `age > 35` にセミコロンをつけると、その部分は文になるので値を返さないユニット型として判定され、コンパイルエラーとなってしまいます。

Rust では、`return` をつけて値を返してもよいのですが、あるブロックの最後でセミコロンを取ると、値を返すことができます。普段はこれを利用するので、`return` はつけません。一方で、ブロックの途中に `if` 式などを書いて、アーリーリターンをさせたい場合などは `return` を明示的に書きます。

また、関数の仮引数と返り値の型は省略できません。必ず書く必要があります。何も返り値を書かなかった場合は、ユニット型として型が設定されます。

関数名の命名規則はスネークケース (たとえば、`fn snake_case(...) -> ... {}`) です。
...
- `fn` を使うと関数宣言をできる。
- ブロックの最後の式のセミコロンを取ると、その値を返すようになっている。
